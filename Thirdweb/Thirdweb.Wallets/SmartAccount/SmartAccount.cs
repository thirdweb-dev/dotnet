...
namespace Thirdweb
{
    public class SmartAccount : IThirdwebAccount
    {
        ...
        protected IThirdwebAccount _personalAccount;
        ...
        public static async Task<SmartAccount> Create(
            ThirdwebClient client,
            IThirdwebAccount personalAccount,
        ...
        {
            if (!await personalAccount.IsConnected())
            {
            ...
            var entryPointContract = await ThirdwebContract.Create(
                client,
                entryPoint,
                chainId,
            ...
            var accountAddress = accountAddressOverride ?? await ThirdwebContract.Read<string>(factoryContract, "getAddress", await personalAccount.GetAddress(), new byte[0]);
            var accountContract = await ThirdwebContract.Create(
                client,
                accountAddress,
                chainId,
            ...
            return new SmartAccount(client, personalAccount, gasless, chainId, bundlerUrl, paymasterUrl, entryPointContract, factoryContract, accountContract);
        }
        ...
        private async Task<UserOperation> SignUserOp(TransactionInput transactionInput, int? requestId = null)
        {
            ...
            var gasEstimates = await BundlerClient.EthEstimateUserOperationGas(_client, _bundlerUrl, requestId, EncodeUserOperation(partialUserOp), _entryPointContract.Address);
        ...
        private async Task<string> SendUserOp(UserOperation userOperation, int? requestId = null)
        {
            ...
            var userOpHash = await BundlerClient.EthSendUserOperation(_client, _bundlerUrl, requestId, EncodeUserOperation(userOperation), _entryPointContract.Address);
        ...
        private async Task<byte[]> GetPaymasterAndData(object requestId, UserOperationHexified userOp)
        {
            if (_gasless)
            {
                var paymasterAndData = await BundlerClient.PMSponsorUserOperation(_client, _paymasterUrl, requestId, userOp, _entryPointContract.Address);
